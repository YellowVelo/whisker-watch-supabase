...
- [x] **Filename collision.** The Archive copy of `0006 Pet Delete Test and Demo Accounts V2.md` was a genuinely different document — an as-built reconciliation memo with its own "Open decisions for Product" list, not a duplicate of the real spec — that happened to share the exact filename with `docs/features/0006 …V2.md`. Renamed at the time so it couldn't be mistaken for the spec itself; the whole Archive folder (including this file) has since been removed from the repo.
- [x] **CLAUDE.md's canonical-spec pointer.** Fixed to point at `docs/features/0012_DailyCheckIn_Vibe_Trends_Specification_v5.md` (the real file) — at the time, a byte-identical `.txt` duplicate also sat in the now-removed Archive folder; that duplicate is gone along with the folder, so this is no longer a live concern.
...
- [x] **The nightly analytics rollup's `checkins_completed` count was silently broken since the Vibe model shipped (2026-07-13) — resolved 2026-08-01.** `compute_daily_analytics_summary()` (migrations 0023/0024) counted completions by querying `analytics_events` for `event_name in ('daily_check_in_marked_normal', 'daily_check_in_marked_changed')` — neither event name was fired anywhere in the current codebase; the actual completion event had been renamed to `vibe_recorded` (`DailyCheckInSheet.jsx:70,131`) the day after the rollup migration shipped, and the rollup function was never updated to match. `app_opened`/`daily_check_in_started`/`daily_check_in_skipped` all still fired correctly throughout, so DAU/new/returning/started/skipped counts were never affected — only `checkins_completed` undercounted. **Fixed by migration 0043**, which also backfilled every existing `analytics_daily_summary` row with the corrected logic. See [0030_Analytics_Test_Account_Filter_Specification_v1.md](features/0030_Analytics_Test_Account_Filter_Specification_v1.md).
