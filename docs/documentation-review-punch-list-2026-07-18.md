...
- [x] **PWA install flow now has a doc:** `docs/features/PWA Feature.md` (2026-07-18) — covers the manifest/service worker (`vite-plugin-pwa`), the Chromium/Android install prompt (Settings row), the iOS Safari manual-install banner, and the offline banner. Also resolves a stale claim in `docs/launch-punch-list.md` P1 (`manifest.json` "missing"/404s — that was true before `vite-plugin-pwa` was added, not true of the current build) and flags a real gap: Firefox and non-Safari iOS browsers get no install nudge at all.
- [x] **Analytics now has a doc:** `docs/features/Analytics Feature.md` (2026-07-18) — covers the full event catalog (~45 live events), the `account_type` tagging, the `pg_cron` nightly/hourly rollup, and access/exposure (SQL-Editor-only, no in-app dashboard). Also documents a retired event found in live data (`menu_pet_profiles_selected`, matching no current Menu item) and explicitly clarifies that this system is **not** a database backup — a real, separate, still-open gap tracked in `docs/launch-punch-list.md` P0 (`pitr_enabled: false`, no backups configured at all).
...
- [x] `docs/launch-punch-list.md` line 26 listed "`manifest.json` is missing" as an open P1 item — already resolved (`vite-plugin-pwa` auto-generates it as of the 2026-07-11 PWA commit). Fixed directly in that file (2026-07-18), alongside writing `docs/features/PWA Feature.md`.
...
- **2 P4** — down from 3; the `manifest.json` cross-file checkbox is now fixed.
