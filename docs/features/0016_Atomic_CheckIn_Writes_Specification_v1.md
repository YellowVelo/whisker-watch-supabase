...
**Date:** 2026-07-25
**Related files:** `src/lib/checkin/checkinClient.js`, `src/components/DailyCheckInSheet.jsx`, `src/components/catchup/CatchUpFlow.jsx`, `src/components/catchup/BulkApplySheet.jsx`, `supabase/migrations/0014_daily_checkins_wellness.sql`, `supabase/migrations/0034_save_daily_check_ins.sql`, `docs/launch-punch-list.md` (P5)
...
- **`checkinClient.js` — implemented as planned, with one addition.** `markGreatDay`, `markOffTough`, and `markGreatDaysBulk` kept their exact current exported names and signatures, and internally call `supabase.rpc('save_daily_check_ins', { payloads: [...] })` as designed. **What actually shipped differently from this spec's original claim:** a new function, `markOffToughBulk(petId, dates, status, selections, source)`, was added for `BulkApplySheet.jsx`'s multi-day apply — it resolves the shared selections once (not once per date, since a bulk-apply session always applies one answer to every selected date), then chunks and calls the RPC the same way `markGreatDaysBulk` does. Two shared helpers, `buildObservationRows` and `rowsToObservationPayload`, were extracted out of `markOffTough` so this new function doesn't duplicate the category-resolution logic a third time. Contrary to this spec's original claim, `BulkApplySheet.jsx`'s import and call site *did* need to change (from calling `markOffTough` once per date, to calling `markOffToughBulk` once) — "no caller needs to change" was only true for `DailyCheckInSheet.jsx` and `CatchUpFlow.jsx`.
- **Chunk size: 20 days per call, as a starting default.** `markGreatDaysBulk` splits its date list into chunks of 20 and calls the RPC once per chunk, sequentially (not parallel) so a failure's exact position is always known — a 180-day gap becomes 9 round trips instead of 180 (or 1). `markOffToughBulk` (see above) gives `BulkApplySheet.jsx`'s multi-day apply the same chunking treatment, though it rarely selects more than a handful of days so it'll almost always be a single chunk in practice. 20 is a reasonable starting point (~1-2 seconds per chunk, ~10-20 seconds worst case total) but isn't a hard product requirement — fine to tune after real use.
...
- **Existing tests must be rewritten, not just left passing.** `src/lib/checkin/checkinClient.persistence.test.js` has 5 tests (covering `markGreatDay`, `markOffTough` x2, `markGreatDaysBulk`) that work today by faking ("mocking") the current separate upsert/delete/insert calls and checking each one happened correctly. Once those functions call the new `save_daily_check_ins` RPC instead, those 5 tests will fail — not because anything is broken, but because they're checking for steps that no longer exist. Rewriting these 5 tests to mock and assert against the single RPC call is part of this work, not a follow-up.
...
1. **[Engineering] Resolved.** Migration number confirmed as `0034`, applied to both `wysker-watch-dev` and production.
2. **[Engineering]** Chunk size of 20 is a starting default, not a tested/tuned number — still open, worth revisiting once this has run against a real 6-month gap (timing) or gets user feedback (does a ~10-20s worst-case wait feel acceptable in practice).
3. **[Engineering] Resolved.** Shipped with no retry/backoff — a failed chunk just stops, same as any other interrupted session, relying on Catch Up's resume-from-any-point design (matches the "leaning toward" default this spec originally proposed).
