...
**Status:** Reverted (2026-07-26). Was implemented and deployed to all three environments the same day, then rolled back after unresolved confusion verifying the frontend messaging behavior in the live app — see migration `0037_revert_prevent_demo_account_writes.sql` and punch list. All three code layers (entityClient.js guard, DB trigger, Edge Function guards) and the client-side pre-checks have been reverted; demo accounts are writable again, same as before this spec. **Superseded by spec `0027`** (2026-07-31), which solves the same underlying concern — demo accounts writing data that shouldn't stick around — with an auto-reset-on-login model instead of blocking writes.
**Date:** 2026-07-25
