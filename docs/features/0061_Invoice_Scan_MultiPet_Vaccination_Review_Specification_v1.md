...
- AI failure shows an error and doesn't open a garbage review screen → Playwright test: mock a 429/500 response, assert the existing `aiErrorText()` toast appears and no review screen opens. (Note: this test's toast-text locator uses `.first()` because fixing an unrelated, pre-existing toast-rendering bug — discovered via this exact test during production smoke testing, since a real toast was never actually reaching the screen — made the real Radix toast component also emit a proper screen-reader announcement alongside the visible text, so the same text now matches two DOM nodes. See `0062_Toast_System_Position_Fix_Specification_v1.md` for the full investigation.)
