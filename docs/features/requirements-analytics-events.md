...
- **Event catalog** — document the full list of event names currently fired (grep `track(` across `src/` to regenerate this list if it drifts; as of 07-13 it includes, grouped by area):
  ...
  - **Multi-Day Catch Up** (spec 0015, added 2026-07-25 — the 2+ day gap flow; `catch_up_started`/`catch_up_completed` above remain the 1-day-only flow): `multi_day_catch_up_started` (Home banner tapped), `multi_day_catch_up_flow_entered` ("Get Started" past the welcome screen), `multi_day_catch_up_feeling_selected` (30+ day long-gap prompt answer — analytics only, never persisted per day), `multi_day_catch_up_day_saved` (one exception day detailed), `multi_day_catch_up_bulk_applied` / `multi_day_catch_up_bulk_apply_abandoned` (applying one set of details to several selected exception days at once), `multi_day_catch_up_completed` ("Finish Catch Up" — the bulk Great Day write for every untouched day), `multi_day_catch_up_dismissed` (closed without finishing; also fires from the entry screen's "Maybe later").
