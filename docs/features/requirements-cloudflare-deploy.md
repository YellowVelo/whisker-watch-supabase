...
- **Confirmed with Lynn (2026-08-09):** deploys go through Cloudflare's own git-integration build (Cloudflare watches the tracked branch and builds on push), followed by a manual promotion step Lynn performs in the Cloudflare dashboard — that promotion step is the "manual-deploy gate." `npm run deploy` (`wrangler deploy`) does exist in `package.json` (has since 2026-07-04, predating this doc) but is a separate local-only command, not part of the shipped promotion pipeline.
