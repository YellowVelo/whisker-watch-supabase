...
- **Two call paths:**
  ...
  Both paths accept an optional `sentByUserId` — see Business Rule 5.
...
- `email_logs`: `recipient_email`, `template_name`, `status` (`pending` | `sent` | `suppressed` | `failed` — `suppressed` added in migration `0032`, see Business Rule 5), `provider_message_id`, `error_code`, `error_message`, `related_entity_type`/`related_entity_id`, `idempotency_key` (unique where non-null), `sent_at`, `created_at`. RLS enabled with **zero policies** — service role bypasses RLS entirely; every other role is denied by default, since there is no client-side insert path.
...
- Test/demo send-suppression is now centralized in `sendEmail()` via `sentByUserId` (done 2026-07-24) — the note that previously lived here about this being future follow-up work is resolved; see Business Rule 5.
