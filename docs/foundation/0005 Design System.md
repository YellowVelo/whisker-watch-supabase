...
**Resolved 2026-08-28, spec 0059:** Stacking order (z-index) is never a hand-picked number. Before this, every fixed/sticky surface in the app (full-screen flows, popups, banners, page chrome, toasts) picked its own z-index and justified it in a local code comment — several explicitly citing another component as precedent, with no enforced relationship between them. That let a popup opened from inside a full-screen flow render invisibly behind it (Catch Up's bulk-apply sheet) and let a system banner sit on top of a popup's own action button (`IosInstallBanner` over a `BottomSheet` footer), both by accident.

12. **One named layering scale, not ad-hoc numbers.** Every fixed/sticky surface must reference the shared scale in `src/lib/zIndex.js` — `chrome → overlay → banner → popup → toast`, low to high — instead of inventing a number. A new full-screen flow or popup picks a name from that list; it does not add its own `z-[N]`. Popups/sheets are defined above both full-screen flows and system banners by construction, so a popup opened from anywhere in the app always wins the stacking contest, not just today's specific cases.
