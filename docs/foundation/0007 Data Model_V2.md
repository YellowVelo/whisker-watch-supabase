...
- Change-Based Logging: `daily_check_ins` + `observations` distinguish "normal," "something changed," and "skipped," per Product Principle 6 ("Missing Data Is Meaningful").
...
| wellness_scores | 0014 | `is_pet_owner()` |
...
One row per pet per day. `id`, `created_by`, `pet_id`, `check_in_date`, `status` (`normal`/`changed`/`skipped`), `completed_at`, `source` (`app`/`notification`/`widget`/`sitter`), `notes`, timestamps. Unique on `(pet_id, check_in_date)`.

3.15 observation_types / 3.16 observation_options
Reference data (not owner-scoped; readable by any authenticated user, writable only by migration/service role). 13 seeded types: appetite, water_intake, bathroom, stool, vomiting, energy, mobility, breathing, itching, behavior, medication_exception, weight, other. Each type has `category`, `species_applicability` (`cat`/`dog`/`both`), `answer_type` (`enum`/`number`/`text`/`boolean`), `baseline_supported`, `score_supported`, `sort_order`. Each enum type has seeded `observation_options` rows with a `severity_score` (negative = deduction from the day's wellness score) — see `src/lib/checkin/scoring.js` and `src/lib/checkin/config.js` for the client-side mirror of this data used to render the Daily Check-In UI.

3.17 observations
The actual owner-reported observations — primary truth for the Daily Check-In system. `id`, `created_by`, `pet_id`, `daily_check_in_id` (nullable FK), `observation_type_id`, `value`, `numeric_value`, `severity_score`, `notes`, `photo_url`, `observed_at`, timestamps.
...
3.19 wellness_scores
Calculated score snapshot, one per pet per day. `id`, `created_by`, `pet_id`, `check_in_date`, `daily_check_in_id`, `score` int (0–100), `trend` (`stable`/`improving`/`monitor`/`declining`/`unknown`), `score_reason_summary`, `created_at`. Unique on `(pet_id, check_in_date)`. Computed client-side by `src/lib/checkin/scoring.js` (`computeDayScore`, `computeTrend`) at check-in time and persisted here — this is the only historical, chartable per-day score series in the schema today.
...
pets (1) ─── (N) pet_foods, food_logs, medications, vaccinations, bloodwork,
              ...
              observations, pet_baselines, wellness_scores
...
- The Daily Check-In flow on Home writes to `daily_check_ins` + `observations` + `wellness_scores`, as before.
- Pet Profile's Wellness Summary rings (Wellness/Appetite/Energy/Symptoms) and the Trends screen's Appetite/Water Intake/Energy cards now all read from `daily_check_ins`/`observations` (`getWellnessRingScores` in `petProfileClient.js`, `getObservationTrend` in `trendsClient.js`) — the old `symptom_logs`-based 30-day recharts trend charts and their quick-log bubbles have been removed from Pet Profile entirely.
- **Weight is the one remaining exception.** `symptom_logs.weight_grams` is still the only place weight is ever written (via `WeightQuickLogSheet` in `PetProfile.jsx`) or read (Pet Profile's Weight card via `getWeightSummary`, and the Trends screen's Weight card via `getWeightTrend`) — there is no `weight` write path through `observations`, even though `observation_types` has a seeded `weight` type. Both surfaces document this as a known limitation ("no true baseline exists yet") rather than papering over it.
- Wellness Score itself is still computed only from `observations`/`daily_check_ins`; it does not read `symptom_logs`.
...
A second on-demand AI call now exists: `getInsightSummary()` in `src/lib/checkin/trendsClient.js` (backing the Trends screen, see Navigation & IA_V4.md and Insights.md), which reuses the same `invokeAI` client helper (`src/api/aiClient.js`) as `PetAIInsights.jsx` — not a separate AI-calling path — but builds its context from the Trends screen's already-fetched Wellness/Appetite/Water/Energy/Weight data instead of raw `symptom_logs`/`bloodwork`. It requires at least 2 non-skipped check-ins in the last 10 days before calling the AI at all (returns `null` otherwise, so the UI can show "Complete more check-ins to unlock AI insights" instead of an empty/failed call). Like `PetAIInsights`, nothing here is persisted — still no `ai_insights` table.

Summary

The data model is relational and owner-scoped via a single shared RLS helper (`is_pet_owner()`, now correctly linking co-owners since migration 0016). The older symptom/baseline system (`symptom_logs`, `pet_onboarding`) and the newer, more structured one (`observations`/`pet_baselines`/`wellness_scores`) have mostly converged — `symptom_logs` is now scoped to weight only — but `pet_baselines` remains unpopulated, and `pet_onboarding`'s fixed-form baseline is still the only baseline data actually in use. Any new feature should state explicitly which system it builds on for weight specifically, and should not assume `pet_baselines` is populated.
