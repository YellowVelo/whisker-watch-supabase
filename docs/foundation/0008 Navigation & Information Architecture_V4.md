...
- The `01 Features/` folder was flattened and renumbered since this document's original version. Feature docs now live under `docs/foundation/` and `docs/features/` — the flat `01 Features/000N Name.md` numbering this document previously referenced no longer reflects the actual folder structure; see `CLAUDE.md` for the current layout.
