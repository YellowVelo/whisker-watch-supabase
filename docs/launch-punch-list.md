...
- [ ] **CI workflow built, branch protection not yet turned on.** `.github/workflows/ci.yml` now runs lint + vitest + build + the Edge Function integration tests (see item below) on every PR to `main` — but nothing in GitHub's repo settings requires it to pass before merge yet, and pushes still go straight to production gated only by the manual-deploy step in the Cloudflare dashboard. Needs GitHub Actions secrets added (`SUPABASE_DEV_URL`, `SUPABASE_DEV_ANON_KEY`, `SUPABASE_DEV_SERVICE_ROLE_KEY`, `SUPABASE_DEV_TEST2_EMAIL`, `SUPABASE_DEV_TEST2_PASSWORD`) before the workflow can run in GitHub, then a few green runs, then flip on required-status-check branch protection.
