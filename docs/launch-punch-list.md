...
- [ ] **The standalone Pet Profile route (`/pet/:petId`, `context="profile"` in `PetProfileContent.jsx`) — the single Vibe icon + Weight line view, and the only place today's completed check-in can be reopened/edited — is unreachable through normal in-app navigation.** Found 2026-07-25 while manually verifying `0016_Atomic_CheckIn_Writes_Specification_v1.md`'s atomic-writes fix: neither Home's pet card (`PetSummaryCard.jsx`, links to `/pet/:id/trends`) nor the Pets-tab card (`ExpandablePetProfileCard`, `context="pets"`, five Wellbeing chips, also linking to `/trends`) nor the Trends page itself ever links to bare `/pet/:petId`. The only two places that route is reached at all are the post-onboarding redirect (`PetOnboarding.jsx:59`, `navigate(`/pet/${petId}?startCheckin=1`)`) and the accept-invite redirect (`AcceptInvite.jsx:78`). Once onboarding is done, there is no button/link/tab a returning owner can tap to reach the one screen that lets them edit an already-saved check-in — confirmed by direct navigation testing, not just a code read. Distinct from the existing item above ("Pets-tab's Wellbeing chips don't launch Daily Check-In") — that one is about the Pets-tab chips routing to Trends instead of a check-in launcher; this one is about the entire edit-a-check-in screen having no discoverable entry point at all.
...
- [ ] **`checkinClient.js`'s multi-step writes aren't truly transactional — sequential network calls, no shared database transaction.** Renamed 2026-07-25: this item previously named `markSkipped`/`saveChangedCheckIn`, but `saveChangedCheckIn` doesn't exist anymore (retired along with the old Normal/Changed model), and `markSkipped` was never the multi-step one — it's a single upsert. The functions this concern actually applies to today are `markGreatDay` and `markOffTough` (each: upsert → delete → bulk-insert, `markOffTough` also a trailing `update()` — 3-4 sequential calls), and now also `markGreatDaysBulk` (added 2026-07-25 for Catch Up's "Finish" action — same pattern, but can span up to ~180 days in one call, so a partial-failure mid-batch now has a bigger blast radius than before). Reordered to fail safe, not bulletproof. A real fix needs a Postgres RPC wrapping the writes — **scoping this as its own spec next.**
