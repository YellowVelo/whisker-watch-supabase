...
- [ ] **Test/demo account email suppression isn't centralized.** Two call sites (`invite-co-owner/index.ts`, `invite-sitter/index.ts`) each independently check `account_type` and skip real sends for `test`/`demo` accounts. The shared `sendEmail()` helper itself still has no such gate — its own comments call this a "deliberately deferred gap," since `sendEmail()` only knows the recipient address, not which account triggered the send. `sendEmail.ts`'s own comment already flags that a second caller needing this guard is "the signal to centralize it" — that signal has now happened twice.
