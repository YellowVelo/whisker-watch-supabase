...
- [ ] **Branded transactional email (signup confirmation, at minimum) lands in spam on at least Yahoo Mail — new, found 2026-07-26.** Confirmed via real signup test: Resend's own delivery webhook shows `delivered_at` populated (the receiving MTA accepted the message), but it arrived in Junk/Spam, not the inbox. Likely a sending-domain reputation/authentication issue (SPF/DKIM/DMARC alignment, domain warm-up) rather than an application bug — DMARC exists in monitor-only mode (`p=none`, see resolved item below) but that alone doesn't improve inbox placement. Needs investigation on the Resend/DNS side before this stops undermining an otherwise-working email feature.
