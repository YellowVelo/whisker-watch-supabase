...
- [ ] **Signup confirmation email isn't branded — in progress, deployed 2026-07-26, pending final verification.** `sign-up`/`confirm-email` Edge Functions built, reviewed, migrated, and deployed to dev/staging/prod per [0021_Branded_Signup_Confirmation_Email_Specification_v1.md](features/0021_Branded_Signup_Confirmation_Email_Specification_v1.md). Remaining before this can be checked off: an actual real-signup-through-click-confirmation-link pass on each environment (Register.jsx already switched to call the new function, but no one has walked the live flow end-to-end yet).
