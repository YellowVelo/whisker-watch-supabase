...
// account_type rarely changes mid-session, and track() fires often
// (many calls during onboarding alone) — cache it per user instead of
// re-querying profiles on every single event.
const accountTypeCache = new Map(); // userId -> { accountType, fetchedAt }
const ACCOUNT_TYPE_CACHE_TTL_MS = 5 * 60 * 1000;

async function getCachedAccountType(userId) {
...
export async function track(eventName, properties = {}) {
  try {
    ...
    const accountType = await getCachedAccountType(userId);
