...
import {
  ArrowLeft, ChevronRight, MoreHorizontal, Share2, Pencil, Trash2, Rainbow,
...
export default function PetProfile() {
  ...
  // Header data loads independently of the rest of the page (Feature Spec
  // §1: "Header data must load immediately when the profile opens") — this
  // flag only guards the pet-row fetch, never the wellness/card batch.
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      wellnessR, ringScoresR, weightR, medsR, foodsR, vaxR, onboardingR,
      timelineR, healthRecordsR, checkInR,
    ] = results;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate(-1)} className="rounded-xl px-4 py-2 text-sm font-semibold" style={{ background: 'rgba(255,255,255,0.08)', color: '#fff' }}>Back</button>
          <button onClick={loadData} className="rounded-xl px-4 py-2 text-sm font-semibold" style={{ background: PALETTE.sky, color: '#0D0F1A' }}>Retry</button>
        </div>
      </div>
    );
  }

  if (headerLoading) return <PetProfileSkeleton onBack={() => navigate(-1)} />;
  if (!pet) return null;
    ...
    <PageTransition>
      <div className="min-h-screen bg-background pb-28">
        ...
        <header className="px-4 flex items-center justify-between" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}>
          <button onClick={() => navigate('/pets')} aria-label="Back to Pets" className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <ArrowLeft className="h-5 w-5 text-white" />
          </button>
          <div className="flex items-center gap-2">
            <button onClick={handleShare} aria-label="Share pet profile" className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <Share2 className="h-5 w-5 text-white" />
            </button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button aria-label="More options" className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <MoreHorizontal className="h-5 w-5 text-white" />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => setEditOpen(true)}><Pencil className="h-4 w-4 mr-2" /> Edit Pet</DropdownMenuItem>
                {!isMemorial && (
                  <DropdownMenuItem onClick={() => setMemorialOpen(true)}><Rainbow className="h-4 w-4 mr-2" /> Move to Rainbow Bridge</DropdownMenuItem>
                )}
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={openDeletePetFlow} disabled={!isOnline} className="text-destructive focus:text-destructive">
                  <Trash2 className="h-4 w-4 mr-2" /> Delete Pet
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        ...
        {shareFeedback && (
          <p role="status" className="text-center text-xs text-white/50 mt-1">{shareFeedback}</p>
        )}

        {/* ── IDENTITY (loads as soon as the pet row is fetched, independent
             of wellness/cards below) ── */}
        <div className="flex flex-col items-center px-5 pt-2 pb-1 text-center">
          ...
          <h1 className="text-[26px] font-bold text-white mt-3 leading-tight">{pet.name}</h1>
        ...
        {weightLogOpen && (
          <WeightQuickLogSheet
            ...
            onSaved={() => { setWeightLogOpen(false); loadData(); }}
          />
        )}

        <EditPetSheet pet={pet} open={editOpen} onOpenChange={setEditOpen} onSuccess={() => { setEditOpen(false); loadData(); }} />
        <MemorialDialog pet={pet} open={memorialOpen} onOpenChange={setMemorialOpen} onSuccess={() => { setMemorialOpen(false); loadData(); }} />
...
function PetProfileSkeleton({ onBack }) {
  return (
    <div className="min-h-screen bg-background pb-28" aria-busy="true" aria-label="Loading pet profile">
      <header className="px-4 flex items-center justify-between" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}>
        <button onClick={onBack} aria-label="Back to Pets" className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <ArrowLeft className="h-5 w-5 text-white" />
        </button>
      </header>
      <div className="flex flex-col items-center px-5 pt-2 pb-1">
        <div className="h-24 w-24 rounded-full animate-pulse" style={{ background: 'rgba(255,255,255,0.08)' }} />
        <div className="h-6 w-32 rounded-full animate-pulse mt-4" style={{ background: 'rgba(255,255,255,0.08)' }} />
        <div className="h-4 w-44 rounded-full animate-pulse mt-2" style={{ background: 'rgba(255,255,255,0.06)' }} />
      </div>
...
// Skeleton for just the Wellness Summary + Navigation Cards region, used
// both by the full-page skeleton (pet not loaded yet) and standalone once
// the header is already showing real data (Feature Spec: skeleton
// placeholders for "Header, Wellness summary, Navigation cards" — the
// header's own skeleton resolves independently of this one).
