...
// Directory of the user's pets, linking to each pet's full Pet Profile
// (`/pet/:petId`) — the only entry point into Pet Profile now that Home's
// wellness tile and the Pets list route into Trends instead. Includes
// sitter-shared pets (not just owned/co-owned) — otherwise a sitter-only
// user would have no remaining path to a pet's flat Pet Profile page at
// all, since PetCard/SharedPetRow on the Pets screen now route to Trends.
export default function PetProfilesMenu() {
    ...
    <PageTransition>
      <div className="min-h-screen pb-28">
        <header className="px-4 flex items-center gap-3" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}>
          ...
          <h1 className="text-[22px] font-bold text-foreground">Pet Profiles</h1>
        </header>
