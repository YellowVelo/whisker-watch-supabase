...
// Directory of the user's pets, linking to each pet's Pet Sitter tab
// (`/pet/:petId/profile?tab=petsit`) so Pet Sitter is reachable from the
// main Menu without first opening a specific pet.
        ...
        <main className="max-w-2xl mx-auto px-4 pt-4 space-y-3">
          {loading ? (
            ...
            pets.map((pet) => (
              <Link
                key={pet.id}
                to={`/pet/${pet.id}/profile?tab=petsit`}
